...
def Result():
	...
	plt.plot(lamst, lamfst,'k', label='Звезда')
	plt.plot(disk_data_1[0], disk_data_1[1],'g', label='1')
	...
	plt.xlabel('$ \\lambda\; [ \mathrm{см}$]')
	plt.ylabel('$ \\lambda F_\\lambda \; [\mathrm{эрг}\,\mathrm{см}^{-2}\,\mathrm{с}^{-1}]$')
	plt.legend()
	plt.show()
